...
fn main() {
    let n = 3;
    let t = 2;
    let (participants, X) = shamir::shamir_keygen(n, t);

    let msg = b"rust is best";

    let signers: Vec<Participant> = participants.iter().take(t).copied().collect();
    ...
    let c = schnorr::compute_challenge(&R, &X, msg);
